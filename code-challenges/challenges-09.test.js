...
const mapCurrentEvents = () => {
  // Solution code here...
  let eventsData = currentEvents.news.map(val => {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let elemCount = arr.reduce((acc, val, idx) => {
    acc++;
    return acc;
  }, 0);
  return elemCount;
...
const returnNames = (arr) => {
  ...
  let namesArr = arr.reduce((acc, val) => {
    acc.push(val.name);
    return acc;
  }, []);
  return namesArr;
...
const calculateAverage = (arr) => {
  ...
  let result = avgArr.sum / avgArr.count;
  return result;
};
...
const extractChildren = (arr) => {
  // Solution code here...
  let childA = arr.filter((val) => {
    if (val.name.match(/[a]/gi)) {
      return val;
    }
  });
  let filterChild = childA.reduce((acc, val, idx) => {
    if (val.children) {
      acc = acc.concat(val.children);
      // acc = [...acc, ...val.children];

      /*Stackoverflow: In general you would use concat when you have two (or more) arrays from arbitrary sources, and you would use the spread syntax in the array literal if the additional elements that are always part of the array are known before. So if you would have an array literal with concat in your code, just go for spread syntax*/ 
    }
    return acc;
  }, []);
  return filterChild;
