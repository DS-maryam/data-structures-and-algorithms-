...
const standardizePhoneNumbers = (arr) => {
  ...
  let result = [];
  arr.forEach(val => {
    let x = val.substring(1, 4);
    let y = val.substring(6, 9);
    let z = val.substring(10, 14);
    result.push(x + y + z);
  });
  return result
};
...
const sortByDay = (arr) => {
  ...
  result.push(mon, tue, wed, thu, fri, sat, sun);
  return result;
};
